...
// `guide` cap: a short paragraph naming the project's REAL on-screen controls + flow, so the model
// can point the child at actual buttons instead of inventing generic ones. Bounded so a forged body
// can't bloat the system prompt.
const GUIDE_MAX = 700;
...
export function sanitizeManifest(raw, screen) {
  ...
  const checks = namedEntries(raw.checks, 4, screen, (e) => ({ readOnly: e.readOnly === true }));
