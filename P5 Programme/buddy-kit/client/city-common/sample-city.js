 ...
 * (The sample ring is larger than the template's default so the 18 mission
 * towers have room to ring the central park inside it.)
 *
 * Clearances are authored in "pre-densify" plan space using the same convention
 * the old sampleCityInfill used: densifyLayout pulls positions toward the
 * content centroid (×0.6) while footprints/roads/parks grow (×1.5), so every
 * authored clearance = (required post-densify distance) / 0.6.
 *
...
// Ring-road geometry. Authored radius 470 (≈ post-densify 282) keeps the
// downtown mission band wide enough for 18 readable towers around the park.
const RING_R = 470;
const RING_N = 48;
const PARK_R = 60;
// Central roundabout ring around the park island. Authored radius 180
// (≈ post-densify 108, half-width ~10.5) clears the park (radius 60 → 90 after
// densify) and sits well inside the mission band (authored 250–420). The cross
// avenue and diagonal spokes terminate on this ring so no road crosses the park.
const ROUND_R = 180;
const ROUND_N = 48;
// Approach roads end on the ring's OUTER edge so their asphalt/markings never
// reach into the ring. Ring class primary → width 14 → densified 21 (half 10.5);
// densified outer edge = 180·0.6 + 10.5 = 118.5 → authored 118.5/0.6 = 197.5.
const ROUND_END = ROUND_R + 17.5;
...
/**
 * Authored clearances (see header). The ×0.6 / ÷0.6 and ×1.5 factors mirror
 * densifyLayout: positions compress toward the centroid by 0.6 while every
 * footprint / road width / park radius grows by 1.5.
 */
function aRoadClear(roadWidth, fp) {
  return ((roadWidth * 1.5) / 2 + fpMax(fp) * 0.75 + 2.5) / 0.6;
}

/**
 * Direction-aware road clearance: how far (authored) a building centre must sit
 * from road segment A→B so that after densify the grown building footprint and
 * grown road ribbon keep a ~2.5 m gap. For angled roads a square building faces
 * the road with a corner, so the half-extent used is the projection of the
 * footprint onto the segment's perpendicular — bigger than fpMax/2 at 45°.
 */
function aRoadClearSeg(roadWidth, fp, x1, z1, x2, z2) {
  ...
  const post = (roadWidth * 1.5) / 2 + ext * 1.5 + 2.5;
  return post / 0.6;
}
function aParkClear(radius, fp) {
  return (radius * 1.5 + fpMax(fp) * 0.75) / 0.6;
}
function aBldClear(fpA, fpB) {
  return ((fpMax(fpA) + fpMax(fpB)) * 0.75 + 3) / 0.6;
}
...
export function sampleCityRoads() {
  const ring = ringRoad(CX, CZ, RING_R, RING_N, 24, 'primary');
  const round = ringRoad(CX, CZ, ROUND_R, ROUND_N, 14, 'primary');
  const diag = [];
  for (const [dx, dz] of [[1, 1], [1, -1], [-1, 1], [-1, -1]]) {
    ...
    diag.push({ points: [[CX + dx * off, CZ + dz * off], [CX + dx * 660, CZ + dz * 660]], width: 10, class: 'secondary' });
  }
  // Cross avenue terminates on the roundabout ring's OUTER edge (radius
  // ROUND_END) at the 4 compass points.
  const cross = [
    { points: [[150, CZ], [CX - ROUND_END, CZ]], width: 16, class: 'primary' },
    { points: [[CX + ROUND_END, CZ], [SCALE - 150, CZ]], width: 16, class: 'primary' },
    { points: [[CX, 150], [CX, CZ - ROUND_END]], width: 16, class: 'primary' },
    { points: [[CX, CZ + ROUND_END], [CX, SCALE - 150]], width: 16, class: 'primary' },
  ];
...
function placeOne({ x, z, fp }, angleDeg, rMin, rMax, rPref, roads, parks, occupied) {
  ...
  const clearAt = (px, pz) => {
    ...
    for (const o of occupied) {
      if (Math.hypot(px - o.x, pz - o.z) < aBldClear(fp, o.fp)) return false;
    }
    return true;
  };
...
export function buildSampleCityAnchors(roads, parks) {
  ...
  for (let i = 0; i < inner.length; i++) {
    ...
    place([inner[i]], a - 4, a + 4, 250, 330, 1);
  }
  for (let i = 0; i < outer.length; i++) {
    const a = rowAngles(33.75 + 4)[i % 9];
    place([outer[i]], a - 4, a + 4, 340, 420, 1);
  }
  ...
  place(skyline.slice(0, 3), 285, 360, 540, 640, 2);
  place(skyline.slice(3, 7), 0, 70, 540, 640, 2);
  place(skyline.slice(7, 12), 75, 135, 540, 640, 2);
  ...
  place(civic.slice(0, 6), 160, 200, 690, 830, 2);
  place(civic.slice(6), 150, 210, 830, 950, 2);
  ...
  place(commercial.slice(0, 6), -45, -8, 700, 840, 2);
  place(commercial.slice(6), 8, 45, 700, 840, 2);
  ...
  place(industrial, 195, 235, 560, 700, 2);
 ...
 * the old sampleCityInfill: uniform ~55 m grid, footprint ~20 m (authored), placed
 * only where the cell clears roads, parks, and every anchor. A few office
...
export function sampleCityInfill(anchors, roads, parks) {
  ...
  const clear = (cx, cz) => {
    ...
    for (const o of occupied) {
      if (Math.hypot(cx - o.x, cz - o.z) < aBldClear(FP, o.fp)) return false;
    }
    return true;
  };
  ...
  for(let gz=MIN;gz<=MAX;gz+=PITCH)for(let gx=MIN;gx<=MAX;gx+=PITCH){
    ...
    candidates.push({gx,gz,score:roadDistance+Math.abs(radius-560)*.13+hash01(gx,gz,73)*70});
  }
...
/**
 * Build the complete example city layout (pre-densify "authored" space — the
 * builder runs densifyLayout on whatever it receives, exactly like a student
