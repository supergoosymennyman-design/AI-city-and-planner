...
export function createFlyingTaxi(scene, opts = {}) {
  ...
  // ground level. When no zone is nearby the taxi holds a safe 50m floor.
  ...
  // up to 220m) pass a high cruise altitude; the HK sim keeps the historical
  // rooftop-descend behavior via the default (low) floor.
  ...
  const api = {
    ...
    board(champion) {
      ...
      // Board at the champion's current height (rooftop or ground).
      state.pos.set(champion.state.pos.x, champion.state.y > 1 ? champion.state.y : 2, champion.state.pos.z);
    ...
    update(dt, input, tNow) {
      ...
      const floor = autoNavActive ? autoNavFloor : (nearZone ? 3 : 50);
