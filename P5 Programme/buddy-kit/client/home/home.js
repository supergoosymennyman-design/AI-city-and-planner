...
import { citySim, FIT_STUDIO_URL, WORKSHOP_URL } from './links.js';
const ICONS = {
...
for (const card of CARDS) {
  ...
  if (card.status === 'live') {
    ...
    el.setAttribute('aria-label', `Open ${card.name}${card.external ? ' in a new tab' : ''}`);
  } else {
  ...
  el.innerHTML = `
    ...
    ${card.external ? '<span class="new-tab-cue">New tab ↗</span>' : ''}
