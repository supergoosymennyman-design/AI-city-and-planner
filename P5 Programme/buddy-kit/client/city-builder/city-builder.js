...
function loadTreePacks(isCurrent = () => true) {
  // Use the richer CC0 Quaternius/Kenney tree set for the city canopy. Keep
  // every pack optional so a missing individual model cannot block city boot.
  const urls = {
...
function addTree(x, z, scale) {
  ...
  if (_treeVariants.length) {
    const v = hashString(`${x}|${z}`) % _treeVariants.length;
...
function scatterStreetTrees() {
  if (!_treeVariants.length) return;
...
async function bootInner() {
  ...
  const startDeferredAssets = () => {
    ...
    ]).then(() => { if (current()) buildTreeVariants(); })
