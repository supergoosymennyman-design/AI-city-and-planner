...
import {TIME_KEY,TIME_PRESETS,validTime} from '../city-common/time-of-day.js';
...
export function createTimeOfDay({scene,renderer,sky,hemi,sun,rim,ground,grasses=()=>[],reducedMotion,getClouds,post={},onPresetChange=()=>{}}){
 let id='day';try{id=validTime(localStorage.getItem(TIME_KEY));}catch{}
 ...
 function setTimeOfDay(value){id=validTime(value);from={...current};for(const k of colors)from[k]=current[k].clone();target=values(id);elapsed=0;started=performance.now();try{localStorage.setItem(TIME_KEY,id);}catch{}onPresetChange(id);update(0);}
 ...
 function update(dt){
  ...
  const horizon=look ? current.horizon.clone().lerp(look.horizon,look.horizonMix ?? .25) : current.horizon;
  const skyColor=look ? current.sky.clone().lerp(look.sky,look.skyMix ?? .25) : current.sky;
  ...
  if(post.saturation)post.saturation.value=current.saturation*(look?.saturation ?? 1);
  if(post.vignette)post.vignette.value=current.vignette+(look?.vignette ?? 0);
 ...
 return {update,setTimeOfDay,setLook(value){
  ...
  update(0);
 },get id(){return id;},get settled(){return elapsed===1;},get bloom(){return current.bloom*(look?.bloomScale ?? 1);},get night(){return current.night;},destroy(){emissives.clear();stars.clear();}};
}
