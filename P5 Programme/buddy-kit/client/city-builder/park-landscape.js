...
export function createParkLandscape(scene,layout,environment,{mobile=false}={}){
 const group=new THREE.Group();group.name='Park landscape';scene.add(group);const paving=[],links=[];
 ...
 for(const pieces of [paving,links]){if(!pieces.length)continue;const geometry=mergeGeometries(pieces,false);pieces.forEach(g=>g.dispose());const material=new THREE.MeshStandardMaterial({color:P.stone,roughness:1});const mesh=new THREE.Mesh(geometry,material);mesh.receiveShadow=false;group.add(mesh);}
