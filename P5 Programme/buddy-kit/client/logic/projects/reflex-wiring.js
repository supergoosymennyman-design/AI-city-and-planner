...
const manifest = {
  ...
  // readOnly declared explicitly — same door-clean reasoning as recycle-eye.js: the sanitizer
  // emits it on every check, and testRun WRITES (courseResult readout), so it is not a read.
  checks: [{ name: 'testRun', label: 'Try the course', readOnly: false }],
