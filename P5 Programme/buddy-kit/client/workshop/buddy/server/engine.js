...
// header comment for why); DEFAULT import rather than createRequire or named imports — see
// turn.js's matching comment for the full WHY (lexer-invisible dual-guard exports + the Worker
// shell bundling this file, where createRequire cannot run).
import actionSchemaMod from '../logic/action-schema.js';
const { validateAction, OPS, groupExistsInState, groupsInState, isDirectAction } = actionSchemaMod;

/** Max tool-loop steps per turn (propose-a-few-things-then-answer, not an unbounded loop). */
const MAX_STEPS = 4;

/**
 * Hard ceiling on ONE model call, end to end. WHY THIS EXISTS (measured live, 2026-08-04): the
 * free default endpoint accepted every POST and never sent a byte back — and with no signal on
 * `streamText`, the `fullStream` await below simply never settled. No throw, no terminal frame,
 * nothing in the logs; the child stared at thinking dots until they gave up. A hung upstream must
 * become a bounded, HONEST failure: the timeout routes through the caller's existing error
 * contract (turn.js's catch — the labelled "couldn't reach my brain" stub if nothing streamed, a
 * best-effort close with the streamed text if something did). 60s is far above any measured real
 * turn on the free models (~1-3s first token) while keeping the worst dead-endpoint wait bounded.
 */
...
const OP_SCHEMAS = {
  ...
  // `group` is OPTIONAL here for the same structural reason `addItems`/`removeItems` make it
  // optional above: whether a given check actually REQUIRES one depends on that check's own
  // `takesGroup` declaration (unknown at schema-definition time), so `validateAction` is still the
  // authoritative enforcement of "must be present" vs "must be absent". Declaring it AT ALL matters
  // for a different reason than usual, though: zod object schemas silently STRIP any key the schema
  // doesn't declare (no `.passthrough()`/`.strict()` used anywhere in this file), and the real AI
  // SDK dispatch parses a model's tool call through exactly this schema before `execute()` ever runs
  // (`doParseToolCall` → `safeParseJSON({text, schema})` → `zodSchema.safeParseAsync`). Without this
  // field, a model told (by ops-instruction.js's vocabulary line) to send `"group":"Glass"` would
  // have it dropped here — invisibly, upstream of `validateAction` and this file's own live-state
  // check — and a group-taking check would be structurally unreachable from a real model no matter
  // how correct the rest of this feature is. See tests/engine.test.js's dedicated parse-path test.
  runCheck: z.object({ name: z.string().min(1), group: z.string().min(1).optional() }),
  undoLast: z.object({}),
...
const OP_DESCRIPTIONS = {
  ...
  runCheck: 'Read findings immediately for a declared read-only check; no permission is needed. Other checks follow the project action policy.',
...
function manifestHint(op, manifest) {
  ...
  if (op === 'runCheck') { const s = list(m.checks, (c) => `"${c.name}" = ${c.label}${c.readOnly === true ? ' (read-only, already supplied findings)' : ''}`); return s ? ` Valid: ${s}.` : ''; }
  return '';
}
...
function findingsText(check, state) {
  ...
  const head = `${label} is a look-only check: running it changes nothing, so there is nothing for the child to approve.`
    + ' It reports only the supplied findings —'
  ...
  if (raw.length === 0) {
    ...
    return `${head} It ran this turn and none of the problems it looks for turned up.`
      + ' Report only that. Explain what this limited check establishes and suggest one useful next experiment. "Nothing I check for turned up" is honest;'
  ...
  for (const op of OPS) {
    ...
    tools[op] = tool({
      ...
      execute: async (input) => {
        ...
        // A group-taking check must aim at a group that exists RIGHT NOW. Structural validation
        // stays in action-schema (its file-header law); THIS is the live-state half, possible here
        // because runEngineTurn now threads projectState (read-tools change). Refusing HERE means no
        // proposal card ever exists for an invented shelf — the model reads why and corrects itself.
        // SAFETY RULE (matches `findingsText`'s own header above, spec §11b.3's "WHAT IS
        // INTERPOLATED, AND NOTHING ELSE"): a tool RESULT is model-visible and can be relayed
        // straight into the child's reply, so it may ONLY ever be built from content that already
        // passed `screen()`. `result.action.group` here is MODEL-SUPPLIED — action-schema.js only
        // checked its SHAPE (a 1..40-char string), never its content — so it must NEVER be echoed
        // back into this string, no matter how tempting for a clearer error message. The real group
        // names below are safe to interpolate ONLY because `sanitizeProjectState`'s `okId` already
        // ran every key kept in `projectState.slots[*].groups` through `screen()` — that is the one
        // and only source this refusal is built from.
        // Shared with the JSON-fallback gate (`turn.js`'s `gatedValidate`) via action-schema, so the
        // two paths that can raise a card cannot drift — they did once, and the fallback (the path
        // the free model actually uses) was the one missing the check.
        if (check && check.takesGroup === true && result.action.group !== undefined
            && !groupExistsInState(projectState, result.action.group)) {
          const realGroups = groupsInState(projectState);
          return `That group doesn't exist right now. The child's groups are: ${realGroups.join(', ') || '(none)'}.`;
        }
        onValidAction(result.action);
        return isDirectAction(result.action, manifest)
          ? 'Queued for the app to apply the requested edit after this reply. No approval click is needed. Do not claim it succeeded yet; the app will report success or refusal.'
          : PROPOSED_MESSAGE;
      },
    });
  }
 ...
 *   honestly that nothing was looked at this turn. `timeoutMs` (default TURN_TIMEOUT_MS) bounds the
 *   whole model call — tests pass a small value to prove a hung provider rejects instead of hanging.
 * @returns {Promise<{text:string, proposedActions:object[], usage:object|null, terminal:'done'|'cut'}>}
 */
export async function runEngineTurn({ model, system, messages, screen, write, finalize, enabledOps, manifest, projectState, timeoutMs = TURN_TIMEOUT_MS }) {
  ...
  const writeToolFrame = (action, source) => {
    ...
    write({ type: 'tool', op: action.op, summary, source, ...(isDirectAction(action, manifest) ? { direct: true } : {}) });
  };
  ...
  // The hung-upstream defence (see TURN_TIMEOUT_MS above) has TWO halves sharing one signal,
  // because `streamText` alone gives no settle guarantee: it forwards `abortSignal` into the
  // provider's fetch but never bounds its own `doStream` await — so a provider that ignores the
  // signal (or an endpoint that accepts and sends nothing, the case measured live) would still
  // hang this function forever. The signal is created HERE, not at module scope: AbortSignal.timeout
  // starts counting at creation, so it must be born with the call it bounds. (Node unrefs its
  // timer, so a finished happy-path turn never holds the process open.)
  const ceilingSignal = AbortSignal.timeout(timeoutMs);
  const ceilingError = () => Object.assign(new Error(`model call hit the ${timeoutMs}ms turn ceiling before finishing`), { name: 'TimeoutError' });
  const result = streamText({ model, system, messages, tools, stopWhen: stepCountIs(MAX_STEPS), abortSignal: ceilingSignal });
  ...
  async function* relayFullStream() {
    for await (const chunk of result.fullStream) {
      ...
      if (chunk.type === 'error') {
        ...
        // FIELDS ONLY, never the whole object: an AI-SDK APICallError's own enumerable
        // `requestBodyValues` IS the child's conversation (system prompt + transcript + message),
        // and stringifying `info` wholesale put its first 300 chars — seen live in the deployed
        // Worker's tail, 2026-08-12 — into a log line of a product that promises no pooled chat
        // content. Same rule as turn.js's catch (which caps/scrubs, but must not RECEIVE a body).
        // A RetryError nests the real APICallError in `errors[]` — surface the last one's fields.
        const last = Array.isArray(info?.errors) && info.errors.length ? info.errors[info.errors.length - 1] : info;
        const summary = { name: info?.name, reason: info?.reason, message: typeof last?.message === 'string' ? last.message.slice(0, 120) : undefined, statusCode: last?.statusCode ?? info?.statusCode, url: last?.url ?? info?.url };
        throw Object.assign(new Error(`model stream error: ${JSON.stringify(summary)}`), {
          statusCode: summary.statusCode,
          url: summary.url,
        });
      }
      // Defence half 1 — the DETERMINISTIC mid-stream case. A fired AbortSignal arrives as a
      // stream PART in AI SDK v5 — `{type:'abort'}` on fullStream, NOT a rejection (verified
      // against the v5 docs' stopping-streams page). Left unhandled the stream would just END, the
      // turn would "complete", and the child would get a blank "…" reply that looks like the buddy
      // shrugged. Throwing INSIDE the iteration (rather than leaving it to the ceiling race below)
      // removes a same-tick ordering ambiguity: the ended stream must never win the race as a
      // clean-looking finish. turn.js's catch turns a nothing-streamed abort into the honest
      // labelled stub, and a mid-reply abort into a best-effort close with the text that did arrive.
      if (chunk.type === 'abort') throw ceilingError();
  ...
  // Defence half 2 — the SETTLE GUARANTEE. If the provider never yields a first chunk at all
  // (doStream pending forever — the dead endpoint, or a provider that ignores abortSignal), the
  // iteration above never runs, so half 1 can never fire. Racing the whole turn against the same
  // signal makes "this function settles within timeoutMs" unconditional — the engine's own
  // contract, not a hope about provider behaviour. On a lost race the dangling iteration is
  // abandoned: the caller's `write` goes quiet after its terminal frame (turn.js's entry guard),
  // and the signal has already told the provider's fetch to hang up. No unhandled rejection: the
  // race attached its handler to this promise the moment it was built.
  const ceiling = new Promise((_, reject) => {
    if (ceilingSignal.aborted) return reject(ceilingError());
    ceilingSignal.addEventListener('abort', () => reject(ceilingError()), { once: true });
  });
  const outcome = await Promise.race([runStreamingTurn({
