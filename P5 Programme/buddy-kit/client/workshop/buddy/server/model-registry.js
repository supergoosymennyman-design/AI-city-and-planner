...
export const PROVIDERS = Object.freeze([
  Object.freeze({
    // keyEnvs was [] until 2026-08-12: Zen's docs ("sign in to OpenCode Zen … copy your API key")
    // issue per-ACCOUNT keys, and account identity is what survives a shared egress IP — the
    // deployed Worker's keyless requests are permanently 429'd (Cloudflare IPs are shared by
    // millions of Workers; Zen rate-limits anonymous callers per IP), while the same request with
    // a teacher's own key is identified by the account instead. `alwaysOn` keeps the zero-config
    // keyless path exactly as it was (localhost dev never needs an account); the key slot makes
    // Zen BYOK-capable, so a teacher can paste THEIR key on a classroom device (spec §5.2 — we
    // ship no keys and restrict no market; the deployer may also set ZEN_API_KEY server-side).
    id: 'zen', label: 'Zen', alwaysOn: true,
    baseURL: 'https://opencode.ai/zen/v1', baseUrlEnv: 'ZEN_BASE_URL', keyEnvs: ['ZEN_API_KEY'],
    note: 'the built-in default — free models keyless from home; bring a Zen account key on shared-IP hosts',
  }),
  Object.freeze({
 ...
 * CHURNS (hy3-free passed 2026-07-20, gone by 07-21; deepseek-v4-flash-free answered for weeks then
 * HUNG on 2026-08-04 — accepted every POST, zero bytes back), so re-check before a session with
 * `node server/scripts/verify-models.mjs`. contextLimit 200000 is Zen's documented default.
 *
 * Swept against the live endpoint 2026-08-04 (`GET /zen/v1/models` + a real completion per id):
 *  - `deepseek-v4-flash-free` hung that morning (accepted POSTs, zero bytes) and was dropped;
 *    RE-ADDED the same evening after the owner reported it back and a live streaming probe passed
 *    (3.8s to first chunks). The proof of exactly why verify-models must run before every class.
 *  - SKIPPED `ling-3.0-flash-free` — was `Router.Unavailable` in the morning sweep; passed the
 *    evening re-probe but stays out until someone wants it. Re-check with verify-models.
 */
export const MODELS = Object.freeze([
  // mimo leads: it is provider.js's DEFAULT_MODEL_ID, and the default belongs at the top of the
  // picker. maker omitted: MiMo's vendor was not verified from a primary source, and a guessed
  // attribution would break the honest-naming rule this file exists to keep.
  Object.freeze({ id: 'mimo-v2.5-free', provider: 'zen', label: 'MiMo v2.5', kidNote: 'quick and playful — the usual buddy brain', contextLimit: 200000 }),
  Object.freeze({ id: 'deepseek-v4-flash-free', provider: 'zen', label: 'DeepSeek V4 Flash', maker: 'DeepSeek', kidNote: 'fast and steady all-rounder', contextLimit: 200000 }),
  ...
  // makers omitted on both below: Zen's own /v1/models reports owned_by "opencode" for everything,
  // and big-pickle is a deliberately unnamed stealth entry — telling a child WHO made it would be
  // a guess, and the honest answer ("nobody says yet") is itself a good AI-literacy beat.
  Object.freeze({ id: 'big-pickle', provider: 'zen', label: 'Big Pickle', kidNote: 'a mystery brain on trial — nobody says who made it yet', contextLimit: 200000 }),
  Object.freeze({ id: 'laguna-s-2.1-free', provider: 'zen', label: 'Laguna S 2.1', kidNote: 'new on the free shelf', contextLimit: 200000 }),
]);
