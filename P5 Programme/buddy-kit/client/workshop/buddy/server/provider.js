...
// WHY mimo: measured live 2026-08-04 — `deepseek-v4-flash-free` accepted every request and never
// answered (zero bytes in 90s) while the other three free models answered in ~1s. The free list
// churns (model-registry.js's own header), so the default must be re-checked before sessions with
// `node server/scripts/verify-models.mjs`; deepseek stays IN the registry (it may come back), it
// just no longer gets to be the model a child lands on with zero setup.
