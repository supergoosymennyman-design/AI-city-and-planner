...
import { createOpenAICompatible } from './ai-compat.js'; // shared seam with the Worker shell — see ai-compat.js
...
import { httpError, meterFor, composeTurnContext, runTurn, runTidyUp, runAsk, runSend } from './turn.js';
// DEFAULT import of the CJS frame encoder — see turn.js's matching comment for the full WHY
// (this replaced createRequire so the same modules bundle for the Worker shell too).
...
export const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && req.url === '/api/ask') {
      // The AI-Machines workshop's Cloud AI block: one question in, one short answer out.
      // runAsk owns its own degradation (deflected/braked/failed all resolve to answer-omitted
      // JSON) — only a malformed question's httpError(400) may surface, same as tidy-up.
      return json(res, 200, await runAsk(await body(req), boot, { brake }));
    }
    if (req.method === 'POST' && req.url === '/api/send') {
      // The Send block: POST a machine's signal to an outside webhook, server-side (no CORS
      // fight, no child origin exposed). runSend degrades to {sent:false, note} on everything
      // except a malformed target, which is the builder's own bug and 400s loudly. The resolver
      // is the SSRF gate's third layer (see runSend's header) — this Node gateway sits on a
      // real LAN, so a name resolving to a private address must be refused before any POST.
      return json(res, 200, await runSend(await body(req), boot, { lookup: dns.lookup.bind(dns) }));
    }
