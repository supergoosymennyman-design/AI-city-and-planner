...
export function toolSummary(a, manifest) {
  ...
  try {
    switch (a.op) {
      ...
      case 'runCheck':
        // Group-taking checks (takesGroup, e.g. p5-01's toughenShelf) name their target — the
        // mid-stream tool line is the child's first sight of the action, and "Run Make practice
        // views" names no shelf. Mirrors project-state.js's actionLabel; `group` was validated
        // against the sanitizer-screened live groups before this summary is ever written.
        return a.group !== undefined
          ? `Run ${labelOf(m.checks, a.name)} — ${a.group}`
          : `Run ${labelOf(m.checks, a.name)}`;
      case 'undoLast':
