...
(function () {
  ...
  /**
   * Does this source have pixels MediaPipe can read? A <video> reports videoWidth, a <canvas>
   * reports width, an <img> reports naturalWidth, an ImageBitmap reports width.
   *
   * WHY THIS IS NOT A videoWidth CHECK ANY MORE (2026-08-27): the model reads any of them —
   * proven by spike against the real .task file — but this guard was written for the one caller
   * that existed, a live camera. That made a crate's canvas silently undetectable, which is what
   * kept the Pose sense off the belt. The kit contract (ADAPTER.md §6) had promised canvas input
   * to interns the whole time.
   *
   * WIDENING ONLY: a <video> must keep behaving exactly as before — web/games/k2-17-point-and-learn
   * ships against this.
   * @returns {boolean} whether detect() should attempt a read
   */
  ...
  /**
   * One VIDEO-mode read of `src`.
   * @param {HTMLVideoElement|HTMLCanvasElement|HTMLImageElement|ImageBitmap} src  anything with pixels
   ...
   */
  HolisticManager.prototype.detect = async function (src) {
    try {
      var lm = await this._ensure();
      if (!lm || this._disposed) return null;
      if (!drawableSize(src)) return null;
      ...
      this._lastTs = ts;
      var res = lm.detectForVideo(src, ts);
      if (!res) return null;
