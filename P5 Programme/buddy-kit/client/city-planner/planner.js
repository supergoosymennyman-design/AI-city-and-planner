...
function keyboardPlace() {
  if(state.tool==='place') {placeBuilding(keyCursor.x,keyCursor.y);announceCursor();toast(L('planner.keyboard.placed',{name:displayName(state.selectedType,currentLang()),n:state.layout.buildings.length}));}
    ...
    else if(Math.hypot(keyCursor.x-roadStart[0],keyCursor.y-roadStart[1])>=20) {pushUndo();state.layout.roads.push({points:[roadStart,[keyCursor.x,keyCursor.y]],width:ROAD_WIDTH.residential,class:'residential'});roadStart=null;updateMetrics();render();}
  } else selectNext(1);
}
...
canvas.addEventListener('keydown',e=>{
  ...
  if(['Enter',' '].includes(e.key)){e.preventDefault();keyboardPlace();return;}
  if(e.key==='Escape'){roadStart=null;state.selectedIdx=-1;renderSelectedInfo();render();return;}
...
function updateMetrics() {
  ...
  if(suggestion)suggestion.textContent=m.problems[0]||t('planner.hint.normal');
  renderSelectedInfo();
  requestRender();
