...
const Transcript = (() => {
  ...
  function renderEntry(entry) {
    ...
    speaker.textContent = entry.role === 'child' ? 'You' : 'Botly';
  ...
  function toggle() { if (!drawerEl) return; drawerEl.classList.contains('open') ? close() : open(); }
  function open() { if (!drawerEl) return; drawerEl.classList.add('open'); renderAll(); }
  function close() { if (!drawerEl) return; drawerEl.classList.remove('open'); }
