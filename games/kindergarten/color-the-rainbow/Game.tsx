...
export const Game: FC<{ ctx: GameContext }> = ({ ctx }) => {
  ...
  // Voice input (STT) — the class SAYS the colour ("AI, this is red!" / answers the quiz).
  // Android-Chrome only; tap/keyboard is always the fallback (iPad has no Web Speech, §5).
  // Privacy (§5c): cloud STT is consent-gated in production; the host can withhold `probe`.
  // One listen per prompt; a tap pre-empts it (the effect re-runs and cancels the in-flight one).
  useEffect(() => {
    const wantsColour =
      (data.state === 'teaching' && data.awaitingColorName) ||
      (data.state === 'game' && !!data.gamePick && !data.gameFeedback);
    if (!wantsColour) return undefined;
    let cancelled = false;
    void (async () => {
      if (!(await ctx.ai.probe('listen'))) return; // no STT here → tap/keyboard only
      if (cancelled) return;
      setListening(true);
      ...
      if (!match) return;
      if (data.state === 'teaching') dispatch({ type: 'ANSWER_COLOUR', colour: match });
      else dispatch({ type: 'GAME_TAP', chosen: match });
    })();
    return () => {
      cancelled = true;
      setListening(false);
    };
  }, [data.state, data.awaitingColorName, data.gamePick, data.gameFeedback, ctx]);
