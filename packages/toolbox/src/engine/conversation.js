/*
 * ConversationManager — Web-Speech STT/TTS + energy-VAD + optional LLM, ported
 * verbatim from source/toolbox/conversation.js (R22 prototype) with only an ESM
 * `export` added. It contains NO third-party CDN (Web Speech / speechSynthesis are
 * browser-native; meSpeak/onnx/vosk paths are local `lib/` references).
 *
 * Exported RAW as an escape hatch for the future `converse` capability. The
 * contract's `AIServices.listenOnce` / `AudioBus.speak` are implemented in
 * src/ai.ts / src/audio.ts (the host's already-hardened impls), NOT by wrapping
 * this manager — its continuous startListening()/onUserSpeech model is the wrong
 * shape for a one-shot promise and would reintroduce the onend-never-fires hang.
 * Types live in conversation.d.ts.
 */
...
ConversationManager.prototype.speak = function(text){
  if(!text) return;
  this._interrupted = false;
  this.speaking = true;
