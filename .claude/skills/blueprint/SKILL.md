...
motion/flash safety, colour-blind-safe cues, and the track's look (its render preview). There is
NO shared UI package — each game owns its UI; the boundary is the **vibe**. The blueprint
**references** the vibe and records only the *lesson-specific* UX (which interactions, which two
channels per action, pacing, any inherent sensory exception) — it never restates button sizes
or colours. The `game-builder` builds the game's own UI to the vibe and the `kid-ux-reviewer`
checks the vibe-match at review time.
...
- [ ] The **UX & design-standard binding** section references `docs/standards/ui-ux-common.md` +
      the track doc (`kindergarten-ui-ux.md` / `primary-ui-ux.md`) + its render preview as the
      **vibe** the game's own UI must match; it records lesson-specific UX only and does **not**
      restate colours, fonts, or target sizes (those are defined by the vibe).
