...
test('road transform is the identity and buildings keep their rendered entrance links', () => {
  ...
  // densifyLayout is geometry-preserving now: 1:1, no translation.
  assert.ok(Math.abs(transform.scale - 1) < 1e-9);
  assert.ok(Math.abs(transform.tx) < 1e-9 && Math.abs(transform.tz) < 1e-9);
  assert.deepEqual(rendered.buildings[0].pos, source.buildings[0].pos, 'buildings are not moved');
