...
- **#1 PREREQUISITE — lesson materials as usable data (◐ LARGELY MET 2026-06-07 for K2).** Originally the lessons lived only in **binary `.docx`/`.pdf`** (`source/kindergarten/`, `source/primary/`) with no IDs/specs. Now: the **`docs/curriculum/lessons.json` registry is built** (K2+K3 1–20 from the PDF index, keyed by `(band, lesson)` — see [docs/curriculum/README.md](curriculum/README.md)), and **19/20 K2 teacher scripts are extracted** to `source/kindergarten/k2-NN-<slug>/lesson.md` (Gate-0-ready), CI-validated by the curriculum gate in `validate-contracts`. **Still open:** the typed `Lesson` enum (derives from the registry); **K2 #10** (PDF-vs-docx conflict, held for the designer); **K3 scripts** (PDF one-liners only — no teacher scripts yet); and **primary** (taxonomy/lesson-cut TBD). So the lesson cut / merge subset / capability set are still partly open for K3 + primary, but K2 is buildable today.
...
- **Day 0 — Prerequisites (block the sprint; see §0a):** ✓ `git init`; ◐ **extract the curriculum from `.docx`/`.pdf`** — DONE for K2 (`docs/curriculum/lessons.json` registry + 19/20 K2 scripts in `source/kindergarten/k2-NN-<slug>/`, band-keyed per [README](curriculum/README.md)); ☐ remaining: typed `Lesson` enum (derives from the registry), K2 #10, K3 scripts, primary cut; decide **deployment topology** (recommend teacher-projects-one for KG), **https vs http**, **the TF.js version**; ✓ monorepo scaffold (`package.json`/workspaces/tsconfig/Vite).
...
|---|---|---|---|
| **Lesson materials not in repo (only binary docs)** | ~~High~~ Low (K2) | High | ◐ MET for K2: `lessons.json` registry + 19/20 K2 scripts extracted + CI curriculum gate (§0a/§10). Remaining: `Lesson` enum, K2 #10, K3 scripts, primary cut. Owner: Education SME |
